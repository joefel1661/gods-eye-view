...
test('Cockpit owns a focused shared Display portal and compact Radio controls', () => {
  ...
  assert.match(
    css,
    /@media \(max-width: 1024px\)\s*\{[\s\S]*?#top-center-actions\s*\{[\s\S]*?right:\s*16px;[\s\S]*?left:\s*auto;[\s\S]*?transform:\s*none;/,
  );
  assert.match(
    css,
    /@media \(max-width: 1024px\)\s*\{[\s\S]*?#style-indicator\s*\{\s*display:\s*none;/,
  );
  assert.match(
    css,
...
test('Global Context uses its dedicated right rail without a duplicate Data Layers row', () => {
  ...
  assert.match(html, /id="global-context-cameras-btn"/);
});

test('Global Context standby keeps Contacts as the default chooser copy', () => {
  ...
  assert.doesNotMatch(match[0], /SPACE MISSIONS/);
});
