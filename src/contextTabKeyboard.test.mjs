...
test('Contacts and Cameras both participate in the ordinary Tab sequence', () => {
  for (const id of [
  ...
  assert.match(
    syncSource,
    /\[\s*this\._globalContextFlightsBtn,\s*this\._globalContextCamerasBtn,?\s*\]/,
  );
...
test('Context transition state preserves focus and Tab availability until settle', () => {
  ...
  try {
    ...
    assert.equal(
      ...
      cameras,
      'busy sync retains focused Cameras',
    );
    for (const button of [contacts, cameras]) {
    ...
    owner._contextSection = 'cameras';
    _syncContextModeButtons.call(owner);
    assert.equal(
      globalThis.document.activeElement,
      cameras,
      'settled sync retains focused Cameras',
    );
    for (const button of [contacts, cameras]) {
      assert.equal(button.disabled, false);
      assert.equal(button.tabIndex, 0);
...
test('Context activation and Clear All never native-disable tabs and guard repeated clicks', () => {
  ...
  assert.doesNotMatch(
    select,
    /_globalContext(?:Flights|Cameras)Btn\.disabled\s*=\s*true/,
  );
  assert.doesNotMatch(
    clear,
    /_globalContext(?:Flights|Cameras)Btn\.disabled\s*=\s*true/,
